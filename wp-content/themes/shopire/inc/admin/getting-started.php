...
function shopire_admin_install_plugin() {
    ...
    if ( ! current_user_can( 'install_plugins' ) ) {
        wp_send_json_error(
            array( 'message' => __( 'Unauthorized', 'shopire' ) ),
            403
        );
    }

    // Nonce verification (must match wp_create_nonce)
    check_ajax_referer( 'shopire_nonce', 'nonce' );
    ...
    $activate = activate_plugin( 'fable-extra/fable-extra.php' );
    ...
    wp_send_json_success(
        array( 'message' => __( 'Plugin installed and activated successfully.', 'shopire' ) )
    );
}