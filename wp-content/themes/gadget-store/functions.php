...
define( 'GADGET_STORE_PARENT_INC_URI', GADGET_STORE_PARENT_URI . '/inc');
    ...
    */
    ...
    if ( ! defined( 'GADGET_STORE_THEME_BUNDLE_URL' ) ) {
        ...
        if ( ! defined( 'GADGET_STORE_DEMO_IMPORT_URL' ) ) {
        define( 'GADGET_STORE_DEMO_IMPORT_URL', esc_url( admin_url( 'themes.php?page=gadgetstore-wizard' ) ) );
    }
...
// Show Admin Notice
function gadget_store_promo_notice() {
    if (get_option('gadget_store_notice_dismissed')) {
        return;
    }

    if (isset($_GET['page']) && in_array($_GET['page'], ['gadgetstore-wizard', 'gadgetshop-wizard'])) {
        return;
    }
    ...
    ?>
    <div class="notice gadget-store-notice is-dismissible">
        <div>
            <h3>
                <span><?php echo esc_html( '(Version: ' . $gadget_store_theme_version . ')' ); ?></span><br>
                <?php esc_html_e( 'Thank you for choosing,', 'gadget-store'); ?>
                <?php echo esc_html( $gadget_store_theme_title ); ?> <?php esc_html_e( '!!!', 'gadget-store'); ?>
            </h3>
            <p><?php esc_html_e( 'Welcome! You can now easily start building your website using our beautiful and user-friendly themes. No need to worry about setup — with our one-click demo importer, your site can look just like the demo in minutes. All the tools and information you need to get started are right here!', 'gadget-store'); ?></p>
            <a class="button button-primary gadget-store-buy-now" href="<?php echo esc_url( GADGET_STORE_DEMO_IMPORT_URL ); ?>" target="_blank">
                <?php esc_html_e( 'One Click Demo Import', 'gadget-store') ?>
            </a>
            <a class="button button-primary gadget-store-bundle-button" target="_blank" href="<?php echo esc_url( GADGET_STORE_THEME_BUNDLE_URL ); ?>">
                <?php echo esc_html__( 'Get All 50+ Themes @ $79', 'gadget-store' ); ?>
            </a>
        </div>
        <div class="gadget-store-image-wrap">
            <img src="<?php echo esc_url(get_stylesheet_directory_uri() . '/screenshot.png'); ?>">
        </div>
        <canvas id="gadget-store-notice-confetti"></canvas>
    </div>
    <?php
}
add_action('admin_notices', 'gadget_store_promo_notice');
...
add_action('after_switch_theme', 'gadget_store_reset_notice_on_activation');
function gadget_store_reset_notice_on_activation() {
    delete_option('gadget_store_notice_dismissed');
}

function gadget_store_enqueue_admin_assets() {
    wp_enqueue_script(
        'gadget-store-confetti',
    ...
    wp_enqueue_script(
        'gadget-store-notice-confetti-js',
...
add_action('admin_enqueue_scripts', 'gadget_store_enqueue_admin_assets');